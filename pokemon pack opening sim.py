...
BASIC_ENERGIES = ["Fire", "Water", "Grass", "Metal", "Dark", "Psychic", "Lightning", "Fighting"]

# --- Core Logic ---

def get_random_card(rarity, card_db):
    """Selects a random card of a given rarity from the provided card database."""
    if rarity in card_db and card_db[rarity]:
        return random.choice(card_db[rarity])
    return None

def open_one_pack(set_data, set_name):
    """Simulates opening a single booster pack from a specific set with a 10-card structure."""
    pack_contents = []
    ...
    # Set-specific logic for special slots
    ...
    elif set_name == "Temporal Forces" and 'ACE SPEC' in card_db and random.random() < 1/8: # ACE SPEC Chance (approx)
    ...
    # Fallback to a regular rare if the chosen rarity has no cards (e.g., no Holo Rares in a set)
...
def format_collection_for_export(collection):
    """Formats the card collection into a string for display and export."""
    if not collection:
    ...
    for card, count in card_counts.items():
        if not isinstance(card, tuple) or len(card) != 5:
            print(f"Skipping malformed card data: {card}")
            continue
...
class PackSimulatorApp:
    def __init__(self, root):
        ...
        ttk.Label(results_frame, text="Full Collection (Export Format)").grid(row=0, column=1, sticky="w", pady=(0, 5))
    ...
    def update_collection_display(self):
        export_text = format_collection_for_export(self.total_collection)
